<template>
  <div
    class="bg-white rounded-2xl border border-[var(--color-border)] overflow-hidden mt-4"
  >
    <div
      class="px-4 py-3 border-b border-[var(--color-border)] bg-[var(--color-bg-secondary)]"
    >
      <h4
        class="text-sm font-bold uppercase tracking-wider text-[var(--color-text-secondary)]"
      >
        To'lovlar tarixi
      </h4>
    </div>
    <div class="divide-y divide-[var(--color-border)]">
      ...
      >
        <div>
          ...
          <div
            class="text-xs text-[var(--color-text-tertiary)] flex items-center gap-2"
          >
            <span>{{
              pay.type === "first_month" ? "1-oylik (to'liq)" : "Ijara haqi"
            }}</span>
