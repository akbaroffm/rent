  ...
  <div class="page bg-white">
    ...
    <div class="">
      <div v-if="conversations.length === 0">
        <EmptyState
          title="Xabarlar yo'q"
          description="Ijarachi bilan suhbat boshlang"
        />
      </div>
...
const conversations = computed(() =>
  chatStore.getConversationsForUser(auth.user?.id || "u2"),
);

function getOtherUser(conv) {
  const otherId = conv.participants.find((p) => p !== (auth.user?.id || "u2"));
