<template>
  <div class="page bg-white min-h-dvh flex flex-col">
    ...
    <div class="flex-1 px-4 py-6">
      <!-- Cards List -->
      <div v-if="auth.cards.length > 0" class="space-y-4 mb-8">
        <div 
          ...
          :class="card.type === 'UZCARD' ? 'bg-gradient-to-br from-blue-600 to-blue-800' : 'bg-gradient-to-br from-orange-500 to-orange-700'"
        >
          <div class="flex justify-between items-start mb-8">
            <span class="text-lg font-black tracking-widest italic">{{ card.type }}</span>
            <button @click="handleRemove(card.id)" class="w-8 h-8 flex items-center justify-center rounded-full bg-white/20 hover:bg-white/30">
              <Trash2 :size="16" />
            </button>
          </div>
          
          <div class="mb-6">
            <p class="text-xs opacity-70 uppercase tracking-tighter mb-1">Karta raqami</p>
          ...
          <div class="flex justify-between items-end">
            ...
            <div v-if="card.active" class="flex items-center gap-1 bg-white/20 px-3 py-1 rounded-full text-[10px] font-bold uppercase">
      ...
      <div v-else class="text-center py-12">
        <div class="w-20 h-20 bg-[var(--color-bg-secondary)] rounded-full flex items-center justify-center mx-auto mb-4">
          <CreditCard :size="32" class="text-[var(--color-text-tertiary)]" />
        </div>
        <h3 class="font-bold text-lg mb-2">Kartalar qo'shilmagan</h3>
        <p class="text-sm text-[var(--color-text-secondary)] mb-6">Xavfsiz to'lovlar uchun bank kartangizni kiriting</p>
      </div>

      <!-- Add New Card Section -->
      <div class="border-t border-[var(--color-border)] pt-8">
        <h3 class="font-bold text-lg mb-4">Yangi karta qo'shish</h3>
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="text-sm font-medium mb-1.5 block">Amal qilish muddati</label>
              <input 
...
const isFormValid = computed(() => {
  return newCard.value.number.replace(/\s/g, '').length === 16 && 
